...
so_default_dynlib default_dynlib[] = {
        ...
        { "__dynamic_cast", (uintptr_t)&__dynamic_cast_soloader },
        { "clock_nanosleep", (uintptr_t)&clock_nanosleep },
        { "sched_getscheduler", (uintptr_t)&sched_getscheduler },
        { "fchmodat", (uintptr_t)&fchmodat },
        { "posix_memalign", (uintptr_t)&posix_memalign },
        { "readlink", (uintptr_t)&readlink_soloader },
        { "sendfile", (uintptr_t)&sendfile },
        { "symlink", (uintptr_t)&symlink },
        { "utimensat", (uintptr_t)&utimensat },
        { "getpwuid", (uintptr_t)&getpwuid },
        { "popen", (uintptr_t)&popen },
        { "pclose", (uintptr_t)&pclose },
        ...
        { "getcwd", (uintptr_t)&getcwd_soloader },
        ...
        { "pathconf", (uintptr_t)&pathconf_soloader },
        { "pipe", (uintptr_t)&pipe },
        { "read", (uintptr_t)&read },
        ...
        { "AMediaFormat_toString", (uintptr_t)&AMediaFormat_toString },
...
static void force_symbol(so_module *mod, const char *symbol, uintptr_t value) {
    for (int i = 0; i < mod->num_reldyn + mod->num_relplt; i++) {
        ...
        if (strcmp(mod->dynstr + sym->st_name, symbol) != 0) continue;
...
void resolve_imports(so_module* mod) {
    ...
    // Once Android libc++ is loaded, prefer its libc++abi RTTI vtables over
    // VitaSDK libsupc++'s vtables. Mixing host vtables with Android libc++'s
    // __dynamic_cast causes null virtual calls during openFrameworks startup.
    const char *cxxabi_vtables[] = {
        "_ZTVN10__cxxabiv117__class_type_infoE",
        "_ZTVN10__cxxabiv120__si_class_type_infoE",
        "_ZTVN10__cxxabiv121__vmi_class_type_infoE",
    };
    for (int s = 0; s < (int)(sizeof(cxxabi_vtables) / sizeof(cxxabi_vtables[0])); ++s) {
        uintptr_t android_vtable = so_symbol(&libcxx_mod, cxxabi_vtables[s]);
        if (!android_vtable) continue;
        for (int i = 0; i < (int)(sizeof(default_dynlib) / sizeof(default_dynlib[0])); ++i) {
            if (strcmp(default_dynlib[i].symbol, cxxabi_vtables[s]) == 0) {
                default_dynlib[i].func = android_vtable;
                break;
            }
        }
    }
    ...
    force_symbol(mod, "__dynamic_cast", (uintptr_t)&__dynamic_cast_soloader);
}
